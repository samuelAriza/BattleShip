...
In practice, these are extended finite state machines (EFSMs)—a more expressive version of traditional FSMs that, in addition to states and transitions, incorporate variables, operations, and conditional logic. This allows for modeling more complex behaviors, such as timeouts, turn switching, and error handling, which are essential in a networked multiplayer game like Battleship. As Kurose and Ross explain in Computer Networking: A Top-Down Approach (7th ed., p.182):

>"We refer to this extended FSM description as an extended finite state machine, since we have added variables (similar to those in programming languages) and operations on those variables, as well as conditional actions involving them. The specification of an extended FSM begins to resemble a programming language specification" [Bochman, 1984].

The server FSM describes the lifecycle of a game session, detailing the management of player registration, ship placement, gameplay, and session termination, including the handling of client disconnections. The client FSM complements this information by describing the client’s behavior, including user interactions, responses to server messages, and the end-of-game menu that allows players to start a new game or exit. Together, these EFSMs provide a comprehensive view of the game’s flow from start to finish, emphasizing protocol-driven state transitions and user-driven actions.

These diagrams abstract away low-level networking details (e.g., socket creation, binding, or message transmission) and instead focus on the logical game state progression. Events such as protocol messages, user inputs, timeouts, or disconnections act as triggers for transitions, and corresponding actions include message sending, game state updates, board display, or session cleanup. This abstraction ensures the diagrams remain accessible and focused tools for understanding the system's core functionality.
